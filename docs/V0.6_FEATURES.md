# 🚀 Version 0.6 - Multi-Worker & Performance Improvements
...
#### Benefits:
- ✅ **Avoid Rate Limits**: Distribute channels across multiple Telegram accounts
- ✅ **Higher Throughput**: Process messages in parallel
- ✅ **Fault Isolation**: One worker crash doesn't affect others
- ✅ **Scalability**: Add more workers as your channel count grows

#### Architecture:
```
Worker Manager
├── Worker 1 (API Account 1)
│   ├── Session: worker_1_session
│   └── Channels: 1-10
├── Worker 2 (API Account 2)
│   ├── Session: worker_2_session
│   └── Channels: 11-20
└── Worker 3 (API Account 3)
...
The `start.sh` script **auto-detects** the mode:
```bash
./start.sh  # Automatically detects single or multi-worker mode!
```

---

## 🎮 Admin Bot Updates (Coming Soon)

Future updates will add:
- 🔧 Spawn/stop workers from Telegram
- 📊 View worker status and health
- ⚙️ Assign channels to workers
...
2. **Edit config.json for multi-worker:**
   - See the "Multi-Worker Mode" example in `config.example.json`
   - Create a `workers` array with multiple worker objects
   - Each worker needs unique API credentials
...
5. **Start the bot (auto-detects mode):**
   ```bash
   ./start.sh
   ```
   The script automatically detects multi-worker mode and starts all workers!
...
### Worker Won't Start:
```
❌ Worker worker_1 failed to start
```
...
```
⚠️ Worker worker_2 is dead (restarts: 4)
```
...
├── Monitor Thread (10s interval)
├── Worker 1 Process
│   └── TelegramForwarder instance
├── Worker 2 Process
│   └── TelegramForwarder instance
└── Worker 3 Process
    └── TelegramForwarder instance
...
|---------|--------------|--------------|
| **Rate Limit** | 20 msg/min | 20×N msg/min |
...
- ✅ Multi-worker startup script
- ⏳ Admin bot worker control (coming soon)
- ⏳ Delay investigation results (pending testing)

---

**Status: 🚧 Development Branch (v0.6)**
